...
<div class="container">
    ...
    <h1 class="mb-5 text-3xl font-bold">Edit Item</h1>
    <form action="{{ route('inventory_update', ['inventory' => $inventory->id]) }}" method="POST">
        @csrf
        <div class="flex">
            <div class="flex flex-col flex-1">
                <label class="form-label" for="name">Product name:</label>
                <input class="rounded p-1 border" value="{{ $inventory->name }}" id="name" name="name" type="text">
            </div>
            <div class="mx-2"></div>
            <div class="flex flex-col flex-1">
        ...
        <div class="flex my-3">
            <div class="flex flex-col flex-1">
                ...
                <input readonly disabled class="p-1 border bg-gray-100 rounded" value="{{ $inventory->overhead->profit }}" id="price" name="price" step="0.01" type="number">
            </div>
            <div class="mx-2"></div>
            <div class="flex flex-col">
                <label for="base">Base Price (In PHP)</label>
                <input class="p-1 border rounded" value="{{ $inventory->overhead->base }}" id="base" name="base" step="0.01" type="number">
            </div>
            <div class="mx-2"></div>
            <div class="flex flex-col">
                <label for="profit">Selling Price (In PHP)</label>
                <input class="p-1 border rounded" value="{{ $inventory->price }}" id="profit" name="profit" step="0.01" type="number">
            </div>
            <div class="mx-2"></div>
            <div class="flex flex-col flex-1">
                <label class="form-label" for="name">Stock Qty.</label>
                <input class="p-1 border rounded" value="{{ $inventory->stock_qty }}" id="stock_qty" name="stock_qty" step="1" type="number">
            </div>
        </div>
